"""Build the MFA Policy PDF from the markdown source.

Writes:
  - /mnt/documents/goldfin-mfa-policy.pdf   (chat artifact)
  - public/downloads/goldfin-mfa-policy.pdf (web download)
"""
...
SRC = Path("docs/plaid/mfa-policy.md")
OUT_DOCS = Path("/mnt/documents/goldfin-mfa-policy.pdf")
OUT_PUBLIC = Path("public/downloads/goldfin-mfa-policy.pdf")
...
def header_footer(canvas, doc):
    ...
    canvas.drawString(0.75*inch, LETTER[1]-0.45*inch, "GoldFin Desk · Multi-Factor Authentication Policy")
    canvas.drawRightString(LETTER[0]-0.75*inch, LETTER[1]-0.45*inch, "v2026-06-26.1")
    canvas.drawCentredString(LETTER[0]/2, 0.45*inch, f"Page {doc.page}  ·  goldfindesk.com/mfa-policy")
    canvas.restoreState()
