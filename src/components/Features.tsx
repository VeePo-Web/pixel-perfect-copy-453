...
const BentoTilt = ({ children, className = "" }: bentoTiltProps) => {
  ...
  const handleMouseMove = (e: MouseEvent) => {
    ...
    const { left, top, width, height } =
      itemRef.current.getBoundingClientRect(); //current element position

    /**
     * relativeX and relativeY are values between 0 and 1
     * that represent the position of the mouse relative to the
     * element's bounding box.
     * tiltX and tiltY are values between -50 and 50 that will
     * be used to rotate the element around its center.
     * The values are calculated to create a natural-looking
     * tilt effect.
     */
