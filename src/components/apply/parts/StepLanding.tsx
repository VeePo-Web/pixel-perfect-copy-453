...
export default function StepLanding({ onStart, showWelcomeBack, onResume }: Props) {
  ...
  return (
    <div className="motion-safe:animate-section-in">
      ...
      <h1 className="font-zentry text-[clamp(2.25rem,5vw,4rem)] leading-[1.02] tracking-tight text-bone">
        {c.headline}
      </h1>
      <p className="mt-6 max-w-xl text-[15px] leading-[1.75] text-bone/65">{c.sub}</p>

      {showWelcomeBack && (
        <div className="mt-6 inline-flex items-center gap-3 rounded-full border border-champagne-200/25 bg-champagne-200/[0.04] px-4 py-2 text-[12.5px] text-bone/80">
      ...
      <div className="mt-9 flex flex-wrap items-center gap-x-6 gap-y-3">
        ...
        <button
          ...
          className="group inline-flex items-center text-[13px] text-bone/70 hover:text-champagne-100 transition-colors"
        >
