import { faq } from "../content";
import { track } from "../analytics";

export default function ComparisonFAQ() {
  return (
    <section
      aria-labelledby="compare-faq-heading"
      className="relative border-b border-ink/[0.06] bg-white"
    >
      <div className="mx-auto max-w-4xl px-6 py-20 md:py-28 lg:px-10">
        <div className="font-general text-[10.5px] uppercase tracking-[0.32em] text-champagne-300/70">
          Objection handling · FAQ
        </div>
        <h2
          id="compare-faq-heading"
          className="mt-3 font-display font-medium text-ink [text-wrap:balance] text-[28px] leading-[1.15] tracking-[-0.02em] sm:text-[36px]"
        >
          What owners usually ask when comparing options.
        </h2>
        <div className="mt-8 divide-y divide-ink/[0.06] rounded-2xl border border-ink/[0.08] bg-white shadow-[0_1px_2px_rgba(11,13,18,0.04)]">
          {faq.map((item, i) => (
            <details
              key={item.q}
              className="group px-5 [&_summary::-webkit-details-marker]:hidden"
              onToggle={(e) => {
                if ((e.currentTarget as HTMLDetailsElement).open) {
                  track("comparison_faq_opened", { index: i, question: item.q });
                }
              }}
            >
              <summary className="flex cursor-pointer items-start justify-between gap-4 py-5 text-[15px] font-medium text-ink">
                <span>{item.q}</span>
                <span
                  aria-hidden
                  className="mt-1 inline-block h-4 w-4 shrink-0 text-ink/55 transition-transform duration-200 group-open:rotate-45"
                >
                  +
                </span>
              </summary>
              <p className="max-w-[68ch] pb-5 text-[14.5px] leading-[1.7] text-ink/65">
                {item.a}
              </p>
            </details>
          ))}
        </div>
        <p className="mt-6 text-[11.5px] leading-relaxed text-ink/40">
          GoldFin Desk does not replace tax, legal, accounting, bookkeeping cleanup, or investment advice.
        </p>
      </div>
    </section>
  );
}
