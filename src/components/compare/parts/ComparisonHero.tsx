        ...
        <div className="grid items-start gap-14 lg:grid-cols-[1.05fr_1.05fr] lg:gap-16">
          <div className="motion-safe:animate-section-in">
            ...
            <h1
              ...
              className="mt-5 max-w-[24ch] font-light text-bone text-[40px] leading-[1.04] tracking-[-0.01em] sm:text-[54px] lg:text-[62px]"
            >
...
function DecisionMap() {
  ...
  return (
    <div className="relative rounded-3xl border border-white/[0.07] bg-white/[0.02] p-6 shadow-[0_30px_120px_-40px_rgba(0,0,0,0.6)] sm:p-8">
      ...
      <div aria-hidden className="mt-5 grid grid-cols-3 items-center text-center text-[10px] uppercase tracking-[0.24em] text-bone/35">
        <span>DIY / Low</span>
...
function Column({ label, items, align }: { label: string; items: string[]; align?: "right" }) {
  return (
    <div className={`flex h-full flex-col rounded-2xl border border-white/[0.06] bg-charcoal-900/40 p-4 ${align === "right" ? "items-end text-right" : ""}`}>
      <div className="text-[9.5px] uppercase tracking-[0.26em] text-bone/40">{label}</div>
      <ul className="mt-3 space-y-2">
        {items.map((i) => (
          <li key={i} className="flex items-center gap-2 text-[12px] text-bone/70">
            {align === "right" ? null : <span className="h-1 w-1 rounded-full bg-white/25" />}
            <span>{i}</span>
            {align === "right" ? <span className="h-1 w-1 rounded-full bg-white/25" /> : null}
          </li>
        ))}
      </ul>
    </div>
...
function FlagshipColumn({ item }: { item: { shortLabel: string } }) {
  return (
    <div className="relative flex h-full flex-col rounded-2xl border border-champagne-200/40 bg-charcoal-900/70 p-4 shadow-[0_25px_70px_-30px_rgba(217,190,130,0.5)]">
      ...
      <div className="mt-3 text-[14px] font-light leading-snug text-bone">{item.shortLabel}</div>
      <ul className="mt-3 space-y-1.5 text-[11.5px] text-bone/70">
        <li>· Structure</li>
