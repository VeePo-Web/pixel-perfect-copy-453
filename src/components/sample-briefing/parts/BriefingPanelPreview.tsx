  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-champagne-200/15 bg-charcoal-900/60 backdrop-blur-sm">
      ...
      <div className="flex items-center justify-between border-b border-white/[0.06] px-5 py-4">
        ...
        <div className="rounded-full border border-white/[0.08] px-2.5 py-1 text-[10px] uppercase tracking-[0.22em] text-bone/55">
          Demo data
        </div>
      </div>
      ...
      {status === "loading" ? (
        <div className="p-6">
          ...
          <div className="mt-6 grid grid-cols-2 gap-3">
            {[0, 1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-20 overflow-hidden rounded-lg border border-white/[0.05] bg-white/[0.02]"
              >
                <div className="h-full w-full motion-safe:animate-shimmer-slow bg-gradient-to-r from-transparent via-white/[0.04] to-transparent" />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="p-6 motion-safe:animate-panel-rise">
          ...
          <div className="mt-5 rounded-lg border border-white/[0.05] bg-white/[0.02] p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {["Cash Movement", "Revenue Trend", "Expense Pattern", "Unusual Spend", "Questions to Review", "Decisions to Consider"].map(
              (label) => (
                <div
                  key={label}
                  className="rounded-lg border border-white/[0.05] bg-white/[0.015] px-4 py-5"
                >
                  <div className="text-[10.5px] uppercase tracking-[0.22em] text-bone/45">{label}</div>
                  <div className="mt-3 h-2 w-3/4 rounded bg-white/[0.05]" />
