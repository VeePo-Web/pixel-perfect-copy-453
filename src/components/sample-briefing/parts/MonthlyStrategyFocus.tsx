  ...
  return (
    <ModuleShell
      ...
      cta={
        <a
          href="#/apply"
          className="inline-flex items-center gap-2 rounded-full border border-champagne-200/40 px-6 py-3 text-[13px] tracking-wide text-bone transition-all duration-400 ease-cinema hover:border-champagne-200 hover:bg-champagne-300/[0.06]"
        >
        ...
        <ol className="divide-y divide-white/[0.05]">
          {business.monthlyFocus.map((item, i) => (
            <li key={item} className="grid grid-cols-[40px_1fr] items-center gap-3 px-5 py-4">
              ...
              <span className="text-[14.5px] text-bone/90">{item}</span>
            </li>
          ))}
        </ol>
      </div>
