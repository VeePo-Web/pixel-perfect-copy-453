...
const TAG_TONE: Record<string, string> = {
  ...
  Recurring: "border-ink/15 text-bone/75",
      ...
      <ul className="grid gap-3 sm:grid-cols-2">
        {business.unusualSpend.map((u) => (
          ...
          >
            ...
            <span
              className={`shrink-0 rounded-full border bg-charcoal-900/60 px-2.5 py-1 text-[10.5px] uppercase tracking-[0.18em] ${TAG_TONE[u.tag] ?? "border-ink/15 text-bone/70"}`}
            >
              {u.tag}
            </span>
          </li>
