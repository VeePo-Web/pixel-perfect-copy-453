      ...
      <div ref={ref} className="mx-auto max-w-6xl px-6 py-20 lg:px-10 lg:py-24">
        ...
        <div className="mt-10">
          ...
          <div className="relative">
            <input
              ...
              className="h-12 w-full rounded-full border border-ink/[0.12] bg-white pl-12 pr-5 text-[13.5px] text-ink shadow-[0_1px_2px_rgba(11,13,18,0.04)] placeholder:text-ink/35 focus:border-champagne-200/40 focus:outline-none focus:ring-2 focus:ring-champagne-200/20"
            />
            <svg
              aria-hidden
        ...
        >
          ...
          <aside className="lg:sticky lg:top-24 lg:self-start">
            <div className="hidden font-general text-[10.5px] uppercase tracking-[0.28em] text-ink/40 lg:block">
              Categories
            </div>
            <nav
          ...
          <div>
            {filtered.length === 0 ? (
              <p className="rounded-2xl border border-ink/[0.08] bg-white p-6 text-[13.5px] text-ink/55 shadow-[0_1px_2px_rgba(11,13,18,0.04)]">
                No questions match that search. Try a different word, or pick a category.
              </p>
            ) : (
              <div className="divide-y divide-ink/[0.06] rounded-2xl border border-ink/[0.08] bg-white shadow-[0_1px_2px_rgba(11,13,18,0.04)]">
                {filtered.map((f) => {
                  const open = openId === f.id;
                  return (
                    <div key={f.id} id={f.id} className="px-6">
                      <button
                        type="button"
                        onClick={() => toggle(f.id, f.q)}
                        aria-expanded={open}
                        aria-controls={`${f.id}-panel`}
                        className="flex w-full cursor-pointer items-start justify-between gap-4 py-5 text-left text-[15px] font-medium text-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-champagne-200/40"
                      >
                        <span>{f.q}</span>
                        <span
                          aria-hidden
                          className={`mt-0.5 inline-block h-4 w-4 shrink-0 text-ink/45 transition-transform duration-200 ${
                            open ? "rotate-45" : ""
                          }`}
                        >
                          +
                      ...
                      {open && (
                        <div
                          ...
                          className="-mt-2 max-w-[68ch] pb-5 text-[14.5px] leading-[1.7] text-ink/65"
                        >
                          {f.a}
                        </div>
                      )}
  ...
  return (
    <button
      ...
      className={`shrink-0 rounded-full border px-3 py-1.5 text-[12.5px] transition-colors lg:w-full lg:rounded-lg lg:text-left lg:px-3 ${
        active
          ? "border-champagne-200/60 bg-champagne-50/60 text-ink"
