  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5 text-[10.5px] text-bone/55">
          <span className={`h-1.5 w-1.5 rounded-full ${diffDot}`} aria-hidden />
          {template.difficulty}
        </div>
      </div>
      <h3 className="mt-2 text-[17px] font-light leading-snug text-bone">{template.name}</h3>
      <p className="mt-1.5 text-[13px] leading-relaxed text-bone/65">{template.description}</p>
      ...
      <div className="mt-4 space-y-1 text-[11.5px] text-bone/55">
        <div>
          <span className="text-bone/40">Best for · </span>
          {template.bestFor}
        </div>
        <div>
          <span className="text-bone/40">Time · </span>
          {template.timeToUse}
        </div>
      </div>
      ...
      <div className="mt-5 flex items-center justify-between gap-3">
        <button
          ...
          className="min-h-[40px] rounded-full bg-gradient-to-b from-champagne-100 to-champagne-300 px-5 py-2 text-[12.5px] font-medium text-charcoal-950 transition-all duration-300 ease-cinema hover:shadow-[0_10px_32px_-12px_rgba(217,190,130,0.6)]"
        >
          {template.ctaText}
        </button>
        <button
