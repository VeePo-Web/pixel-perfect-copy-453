  ...
  return (
    <ModalShell open={!!template} onClose={onClose} labelledBy="preview-heading" size="panel">
      ...
      <div className="px-6 py-6">
        <h2 id="preview-heading" className="text-[24px] font-light leading-snug text-bone">
          {template.name}
        </h2>
        <p className="mt-2 text-[13.5px] text-bone/65">{template.description}</p>
        ...
        <div className="mt-6 rounded-xl border border-ink/[0.07] bg-ink/[0.02] p-4">
          ...
          <p className="mt-2 text-[13.5px] leading-relaxed text-bone/80">{template.decisionLine}</p>
        </div>
        ...
        <div className="mt-6 flex flex-col gap-3">
          <button
            type="button"
            onClick={onGet}
            className="rounded-full bg-gradient-to-b from-champagne-100 to-champagne-300 px-6 py-3 text-[13px] font-medium text-charcoal-950 transition-all duration-300 ease-cinema hover:shadow-[0_14px_40px_-12px_rgba(217,190,130,0.6)]"
          >
            {template.ctaText}
          </button>
          <a
...
function Pair({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <dt className="text-[10.5px] uppercase tracking-[0.24em] text-bone/40">{label}</dt>
      <dd className="mt-1 text-bone/80">{value}</dd>
    </div>
  );
}
