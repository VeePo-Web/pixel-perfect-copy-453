          ...
          <table className="w-full border-collapse text-left text-[13px]">
            ...
            <thead>
              <tr className="border-b border-ink/[0.07] text-[11px] uppercase tracking-[0.24em] text-bone/55">
                ...
                >
                  <div className="text-bone">Bookkeeper</div>
                  <div className="mt-1 text-[10px] tracking-[0.22em] text-bone/45">
                    Clean records
                  </div>
                </th>
                <th
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-bone">Monthly Finance Desk</span>
                  </div>
                ...
                <th
                  ...
                  onMouseLeave={() => setHovered(null)}
                >
                  <div className="text-bone">Fractional CFO</div>
                  <div className="mt-1 text-[10px] tracking-[0.22em] text-bone/45">
            ...
            <tbody>
              {tableRows.map((r) => (
                <tr key={r.id} className="border-b border-ink/[0.05] align-top last:border-b-0">
                  ...
                  <td className={`px-5 py-5 text-bone/75 ${colClass("bookkeeper")}`}>
                    {r.bookkeeper}
                  </td>
                  <td className={`px-5 py-5 text-bone/85 ${colClass("mfd", true)}`}>{r.mfd}</td>
                  <td className={`px-5 py-5 text-bone/75 ${colClass("cfo")}`}>{r.cfo}</td>
                </tr>
              ))}
              <tr className="align-top">
                ...
                <td className={`px-5 py-6 ${colClass("mfd", true)}`}>
                  <a
                    ...
                    className="inline-flex rounded-full bg-gradient-to-b from-champagne-100 to-champagne-300 px-4 py-2 text-[12px] font-medium text-charcoal-950 transition-all duration-300 ease-cinema hover:shadow-[0_12px_40px_-12px_rgba(217,190,130,0.55)]"
                  >
                    {tableCTAs.mfd.label}
                  </a>
                </td>
            ...
            return (
              ...
              >
                <summary className="flex cursor-pointer items-center justify-between gap-3 px-5 py-4">
                  <div>
                    <div className="text-[14.5px] text-bone">{label}</div>
                    <div
                      className={`mt-0.5 text-[10.5px] uppercase tracking-[0.22em] ${
                        isMfd ? "text-champagne-200/80" : "text-bone/45"
                      }`}
                    >
                      {role}
                    </div>
                  ...
                  <span
                    aria-hidden
                    className="text-[18px] leading-none text-bone/55 transition-transform duration-300 group-open:rotate-45"
                  >
                    +
                  </span>
                </summary>
                <div className="grid gap-3 border-t border-ink/[0.06] px-5 py-4 text-[13px]">
                  {tableRows.map((r) => (
                    <div key={r.id}>
                      ...
                      <p className="mt-1 leading-relaxed text-bone/80">
