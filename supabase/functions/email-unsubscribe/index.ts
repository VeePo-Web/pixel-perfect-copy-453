// Public one-click unsubscribe for advisory emails. Token-gated (no session).
//   GET  -> flips the flag, returns a small branded confirmation page.
//   POST -> RFC 8058 List-Unsubscribe one-click; flips the flag, returns 200.
// The bi-weekly cron (report-delivery.ts) reads email_preferences and suppresses
// any user who has unsubscribed, so this is the single authority for opt-out.
...
async function applyUnsubscribe(token: string): Promise<boolean> {
  ...
  const { data, error } = await admin
    ...
    .update({ advisory_report_enabled: false, unsubscribed_at: new Date().toISOString() })
...
Deno.serve(async (req) => {
  ...
  const invalid = "This unsubscribe link is invalid or has expired. You can manage email preferences in your GoldFin Desk settings.";
  ...
  return ok
    ? page("You are unsubscribed", "You will no longer receive bi-weekly advisory emails. Your reports are still available any time inside GoldFin Desk.")
